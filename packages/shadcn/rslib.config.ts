...
(function shadcn() {
  commands.rm("src/hooks");
  commands.rm("src/components");

  commands.exec("npx shadcn add --all");

  let index = "";

  globbySync("src/hooks/*.ts").forEach(
    (p) => {
      const { dir, name } = path.parse(p);
      const filePath = path.join(dir.replace("src", "."), name);
  ...
  globbySync("src/components/ui/*.tsx").forEach(
    (p) => {
      const { dir, name } = path.parse(p);
      const filePath = path.join(dir.replace("src", "."), name);
